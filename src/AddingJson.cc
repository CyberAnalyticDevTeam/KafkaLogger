...
bool AddingJSON::Describe(ODesc* desc,
							int num_fields,
...
bool AddingJSON::Describe(ODesc* desc, Value* val, const string& name) const
	{
	if ( ! val->present )
		return true;
...
threading::Value* AddingJSON::ParseValue(const string& s, const string& name, TypeTag type, TypeTag subtype) const
	{
	GetThread()->Error("JSON formatter does not support parsing yet.");
	return NULL;
	}

void AddingJSON::SurroundingBraces(bool use_braces)
	{
